...
SET @endtime = GETDATE();
    PRINT'>> Load Duration: '+ 
        CAST(DATEDIFF(SECOND,@starttime,@endtime)AS NVARCHAR) + 'seconds';
PRINT'----------------------------';
...
SELECT
    ...
    CASE
        ...
        WHEN UPPER(TRIM(REPLACE(cntry,CHAR(13),''))) = '' OR cntry IS NULL THEN 'n/a'
        ELSE UPPER(TRIM(REPLACE(cntry,CHAR(13),'')))
    END AS cntry
